"""Init file (v2.2.1) for P2000 integration."""
...
async def async_setup_entry(hass: HomeAssistant, entry: ConfigEntry) -> bool:
    ...
    _LOGGER.debug("P2000 v2.2.1: async_setup_entry for %s", entry.title)
...
async def async_unload_entry(hass: HomeAssistant, entry: ConfigEntry) -> bool:
    ...
    _LOGGER.debug("P2000 v2.2.1: async_unload_entry for %s", entry.title)
    return await hass.config_entries.async_forward_entry_unload(entry, "sensor")


async def _async_reload_entry(hass: HomeAssistant, entry: ConfigEntry) -> None:
    """Reload the config entry when options are updated."""
    _LOGGER.debug("P2000 v2.2.1: options changed, reloading entry %s", entry.title)
